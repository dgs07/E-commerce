...
function getproducts(){
    ...
    while($row = mysqli_fetch_assoc($result_query)){
      ...
      <div class='card'>
        <img src='./admin/product_images/$product_image1' class='card-img-top w-100 h-200px object-fit-contain' alt='$product_title'>
          <div class='card-body'>
...
function get_unique_brands(){
    ...
    $num_of_rows = mysqli_num_rows($result_query);
    if($num_of_rows==0){
        echo "<h2 class='text-center text-danger'>selected brand is not available</h2>";
    }
    while($row = mysqli_fetch_assoc($result_query)){
      $product_id = $row['product_id'];
      $product_title = $row['product_title'];
